---
name: EricStack Knowledge Base Index
created: 2026-08-15
---

# EricStack Knowledge Base Index

> Last updated: 2026-08-15

## Skills (27 total)

### Process Discipline (11)
| Skill | Description |
|---|---|
| [[erics-process-code-review]] | PR纪律审查（coverage、prose、invariant） |
| [[erics-process-find-simplifications]] | 简化候选项发现与 Agent Note 合并 |
| [[erics-process-prose-standard]] | prose 完整性与编辑纪律 |
| [[erics-process-trim-cot-leakage]] | CoT 泄漏检测与修剪 |
| [[erics-process-doc-standards]] | 文档写作标准 |
| [[erics-process-doc-site-sync]] | VitePress 文档站同步 |
| [[erics-process-translate-docs]] | 中英双语配对文档 |
| [[erics-process-archive-agent-notes]] | Agent Note 生命周期管理 |
| [[erics-process-pre-push-checks]] | 推前最小化检查 |
| [[erics-process-merging-stacked-prs]] | GitHub PR stack 落地 |
| [[erics-process-record-browser-gif]] | UI 演示 GIF 录制 |

### Engineering Abilities (16)
| Skill | Description |
|---|---|
| [[erics-ability-plan-eng-review]] | 工程计划审查 |
| [[erics-ability-plan-ceo-review]] | CEO 级产品审查 |
| [[erics-ability-devex-review]] | 开发者体验审查 |
| [[erics-ability-investigate]] | 四阶段调试流程 |
| [[erics-ability-cso]] | 安全审计 |
| [[erics-ability-health]] | 代码质量仪表盘 |
| [[erics-ability-benchmark]] | 性能回归检测 |
| [[erics-ability-benchmark-models]] | 跨模型基准对比 |
| [[erics-ability-autoplan]] | 全链路审查管道 |
| [[erics-ability-office-hours]] | YC Office Hours 六问 |
| [[erics-ability-spec]] | 模糊意图→可执行 spec |
| [[erics-ability-retro]] | 周迭代回顾 |
| [[erics-ability-context-save]] | 进度保存 |
| [[erics-ability-context-restore]] | 进度恢复 |
| [[erics-ability-upgrade]] | 版本与上游同步检查 |

## Key Decisions

- [[decision/loopx-architecture]] — LoopX goal lifecycle design
- [[decision/skill-naming]] — erics-process vs erics-ability 分类原则
- [[decision/brand-unification]] — EricStack 品牌统一方案

## Concepts

- [[concepts/agent-notes]] — Agent Note 是什么、何时写、如何维护
- [[concepts/loop-engineering]] — Loop Engineering 核心理念
- [[concepts/Boil-the-ocean]] — 完整性原则

## Sources

- [[sources/INTEGRATION.md]] — 双库整合方案完整文档
- [[sources/erics-skills-index]] — 技能索引

## Queries

- [[queries/when-to-use-process-vs-ability]] — 何时用 process skill vs ability skill
- [[queries/how-to-add-new-skill]] — 如何新增一个 skill
