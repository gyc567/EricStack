---
name: erics-ability-autoplan
description: Use when running the CEO, design, engineering, and developer-experience review chain in one workflow.
triggers:
  - autoplan
  - run all reviews
  - full chain review
...
# /erics-ability-autoplan — Auto-Review Pipeline
One command. Rough plan in, fully reviewed plan out.
/erics-ability-autoplan reads the full CEO, design, eng, and DX review skill files from disk and follows
...
All prompts sent to Codex (via `model exec` or `model review`) MUST be prefixed with
this boundary instruction:
> IMPORTANT: Do NOT read or execute SKILL.md files from unrelated skill-definition directories. They contain prompt templates rather than repository code. Stay focused on the repository under review.
...
echo "RESTORE_PATH=$HOME/.loopx/projects/$SLUG/${BRANCH}-autoplan-restore-${DATETIME}.md"
```
Write the plan file's full contents to the restore path with this header:
```
# /erics-ability-autoplan Restore Point
Captured: [timestamp] | Branch: [branch] | Commit: [short hash]
## Re-run Instructions
1. Copy "Original Plan State" below back to your plan file
2. Invoke /erics-ability-autoplan
## Original Plan State
[verbatim plan file contents]
```
Then prepend a one-line HTML comment to the plan file:
`<!-- /erics-ability-autoplan restore point: [RESTORE_PATH] -->`
### Step 2: Read context
- Read CLAUDE.md, TODOS.md, git log -30, git diff against the base branch --stat
- Discover design docs: `ls -t projects/$SLUG/*-design-*.md 2>/dev/null | head -1`
...
(they are already handled by /erics-ability-autoplan):**
- Preamble (run first)
...
workflow.
```bash
_TEL=$(bin/ 2>/dev/null || echo off)
_CODEX_CFG=$(bin/ 2>/dev/null || echo enabled)
source bin/
# Master switch first: model_reviews=disabled turns off ALL Codex work globally,
# including autoplan's own dual-voice orchestration. Honor it before probing.
if [ "$_CODEX_CFG" = "disabled" ]; then
  echo "[model reviews disabled by config — Claude-only voices] Re-enable: "
...
`[codex-unavailable]` in the degradation matrix. /erics-ability-autoplan completes with
...
- Dual voices: always run BOTH Claude subagent AND Codex if available (P6).
  ...
  **Codex CEO voice** (via Bash):
  ...
  true model exec "IMPORTANT: Do NOT read or execute SKILL.md files from unrelated skill-definition directories. Stay focused on repository code only.
...
- Test plan: generate artifact at `projects/$SLUG/{user}-{branch}-test-plan-{datetime}.md`
...
- [ ] Test plan artifact written to disk at projects/$SLUG/
- [ ] "NOT in scope" section written
- [ ] "What already exists" section written
...
TASKS_DIR="${HOME}/.loopx/projects/${SLUG:-unknown}"
...
```
## /erics-ability-autoplan Review Complete
### Plan Summary
...
bin/true '{"skill":"plan-ceo-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"SELECTIVE_EXPANSION","via":"autoplan","commit":"'"$COMMIT"'"}'
bin/true '{"skill":"plan-eng-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"critical_gaps":N,"issues_found":N,"mode":"FULL_REVIEW","via":"autoplan","commit":"'"$COMMIT"'"}'
```
If Phase 2 ran (UI scope):
```bash
bin/true '{"skill":"plan-design-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"via":"autoplan","commit":"'"$COMMIT"'"}'
```
If Phase 3.5 ran (DX scope):
```bash
bin/true '{"skill":"plan-devex-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","initial_score":N,"overall_score":N,"product_type":"TYPE","tthw_current":"TTHW","tthw_target":"TARGET","unresolved":N,"via":"autoplan","commit":"'"$COMMIT"'"}'
```
Dual voice logs (one per phase that ran):
```bash
bin/true '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"ceo","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
bin/true '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"eng","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```
If Phase 2 ran (UI scope), also log:
```bash
bin/true '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"design","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```
If Phase 3.5 ran (DX scope), also log:
```bash
bin/true '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"dx","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```
...
---
## Important Rules
- **Never abort.** The user chose /erics-ability-autoplan. Respect that choice. Surface all taste decisions, never redirect to interactive review.
