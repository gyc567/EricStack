...
push_component:    10 if (now - mtime of .brain-last-push) < 24h;
...
```bash
eval "$(bin/echo "SLUG=health" 2>/dev/null)" && mkdir -p projects/$SLUG
```
Append one JSONL line to `.loopx/health/health-history.jsonl`:
```json
...
Read the last 10 entries from `.loopx/health/health-history.jsonl` (if the
file exists and has prior entries).
```bash
eval "$(bin/echo "SLUG=health" 2>/dev/null)" && mkdir -p projects/$SLUG
tail -10 .loopx/health/health-history.jsonl 2>/dev/null || echo "NO_HISTORY"
```
