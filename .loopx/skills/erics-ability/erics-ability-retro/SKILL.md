...
```bash
[ -f retro-context.md ] && echo "RETRO_CONTEXT_FOUND" || echo "NO_RETRO_CONTEXT"
```
If `RETRO_CONTEXT_FOUND`: read `retro-context.md`. This file is user-authored and may contain meeting notes, calendar events, decisions, and other context that doesn't appear in git history. Incorporate this context into the retro narrative where relevant.
...
cat greptile-history.md 2>/dev/null || true
...
cat analytics/skill-usage.jsonl 2>/dev/null || true
...
You (dev)              32   +2400/-300   src/
alice                    12   +800/-150    app/services/
bob                       3   +120/-40     tests/
```
Sort by commits descending. The current user (from `git config user.name`) always appears first, labeled "You (name)".
**Greptile signal (if history exists):** Read `greptile-history.md` (fetched in Step 1, command 8). Filter entries within the retro time window by date. Count entries by type: `fix`, `fp`, `already-fixed`. Compute signal ratio: `(fix + already-fixed) / (fix + already-fixed + fp)`. If no entries exist in the window or the file doesn't exist, skip the Greptile metric row. Skip unparseable lines silently.
...
**Skill Usage (if analytics exist):** Read `analytics/skill-usage.jsonl` if it exists. Filter entries within the retro time window by `ts` field. Separate skill activations (no `event` field) from hook fires (`event: "hook_fire"`). Aggregate by skill name. Present as:
```
| Skill Usage | /ship(12) /qa(8) /review(5) · 3 safety hook fires |
```
If the JSONL file doesn't exist or has no entries in the window, skip the Skill Usage row.
**Eureka Moments (if logged):** Read `analytics/eureka.jsonl` if it exists. Filter entries within the retro time window by `ts` field. For each eureka moment, show the skill that flagged it, the branch, and a one-line summary of the insight. Present as:
```
| Eureka Moments | 2 this period |
```
If moments exist, list them:
```
  EUREKA /erics-ability-office-hours (branch: feature/auth-rethink): "Session tokens don't need server storage — browser crypto API makes client-side JWT validation viable"
...
```bash
bin/true '{"skill":"retro","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
```
**Note:** Only include the `greptile` field if `greptile-history.md` exists and has entries within the time window. Only include the `backlog` field if `TODOS.md` exists. Only include the `test_health` field if test files were found (command 10 returns > 0). If any has no data, omit the field entirely.
...
cat projects/$SLUG/*-reviews.jsonl 2>/dev/null | grep '"skill":"ship"' | grep '"plan_items_total"' || echo "NO_PLAN_DATA"
```
...
Run the discovery:
```bash
$DISCOVER_BIN --since "<window>" --format json 2>/tmp/erics-discover-stderr
```
Read the stderr output from `/tmp/erics-discover-stderr` for diagnostic info. Parse the JSON output from stdout.
...
next=$((existing + 1))
```
Use the Write tool to save JSON to `retros/global-${today}-${next}.json`:
```json
{
  "type": "global",
  "date": "2026-03-21",
...
- **Global mode:** Does NOT require being inside a git repo. Saves snapshots to `retros/` (not `.context/retros/`). Gracefully skip AI tools that aren't installed. Only compare against prior global retros with the same window value. If streak hits 365d cap, display as "365+ days".
