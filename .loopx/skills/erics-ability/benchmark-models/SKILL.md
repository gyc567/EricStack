...
# /erics-ability-benchmark-models — Cross-Model Skill Benchmark
You are running the `/erics-ability-benchmark-models` workflow. Wraps a model benchmark binary with an interactive flow that picks a prompt, confirms providers, previews auth, and runs the benchmark.
...
If A: list top-level skills that have SKILL.md files (from `find . -maxdepth 2 -name SKILL.md -not -path './.*'`), ask the user to pick one via a second AskUserQuestion. Use the picked SKILL.md path as the prompt file.
...
- **Options:**
  - A) Save to `.loopx/benchmarks/<date>-<prompt-slug>.json`. Completeness: 10/10.
