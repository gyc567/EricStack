...
- **Merged-PR and issue citations inside Agent Notes and postmortems** — sanctioned evidence per the 's change-story routing.
...
1. Scope and exclusions per [erics-process-prose-standard](../erics-process-prose-standard/SKILL.md): require an explicit scope; never touch `vendor/`, `.agents/notes/archived/`, or recorded fixtures and snapshots — recorded model output and sealed history keep their original voice.
